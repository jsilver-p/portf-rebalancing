...
def render_mpop_detail():
    ...
    special = random.random() < 0.25  # 현금성자산(증권사) 행 (pnl 0, cost=value)
    if special:
        v = random.randint(100000, 9000000)
        rows.insert(0, (f"현금성자산({random.choice(KR_BROKERS)})", "현금", v, v, 0, None))
    ...
    gt = []
    for name, cls, value, cost, pnl, qty in rows:
        gt.append([acct_label, acct_short, name, cls, "KRW", qty, None, value, cost, pnl, 1])
    if cash is not None:
        gt.append([acct_label, acct_short, "현금잔고(예수금)", "현금", "KRW",
...
def render_mpop_accounts():
    ...
    d.text((40, ky), "KOSDAQ", font=F(34, True), fill=DARK)
    d.text((240, ky), f"{ks:,.2f}", font=F(34), fill=pnl_color(kd))
    d.text((440, ky), f"{'▲' if kd>=0 else '▼'}{abs(kd):.2f} ({kd/ks*100:+.2f}%)",
           font=F(34), fill=pnl_color(kd))
    d.rectangle([0, H - 280, W, H], fill=NAVY)
